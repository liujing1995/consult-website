...
layui.define(function (exports) {
    ...
    var cascader = {
        ...
        render: function (param) {
            ...
            changeOnSelectSrh = param.changeOnSelectSrh;//点击搜索项
            var isFirst = true;
            // 如果渲染过重新渲染
            var $elem = $(elem);
                    ...
                    if (reqSearch) {  // 异步搜索
                        ...
                        for (var i = 0; i < allList.length; i++) {
                            ...
                            if (item.label.indexOf(value) > -1) {
                                item.text = item.label.replace(/\s+/g, "").replace(/\//g,",")
    ...
    var renderSearchList = function ($search, dataList, notFoundText) {
        ...
        if (dataList.length == 0) {
            ...
            for (var i = 0; i < dataList.length; i++) {
                ...
                htmlStr += '<div class="ew-cascader-search-list-item' + className + '" data-text="'+item.text+'" data-value="' + item.value + '">' + item.label + '</div>';
            }
        }
        $search.html(htmlStr);
        $('.ew-cascader-search-list-item').click(function(){
            if(typeof(changeOnSelectSrh)=='function')changeOnSelectSrh($(this).attr('data-value'),$(this).attr('data-text'));
        })
