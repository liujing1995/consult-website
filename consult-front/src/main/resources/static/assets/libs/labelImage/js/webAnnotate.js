...
class LabelImage {
	...
	Initial = () => {
		...
		_nodes.canvas.addEventListener('dblclick', this.CanvasDblclick);
		...
		if(_nodes.crossLine)_nodes.crossLine.addEventListener('click', this.CrossHairSwitch);
		if(_nodes.labelShower)_nodes.labelShower.addEventListener('click', this.IsShowLabels);
		if(_nodes.screenShot)_nodes.screenShot.addEventListener('click', this.ScreenShot);
		if(_nodes.screenFull)_nodes.screenFull.addEventListener('click', this.IsScreenFull);
		if(_nodes.historyGroup)_nodes.historyGroup.addEventListener('click', this.HistoryClick);
		...
		if(_nodes.canvas)_nodes.canvas.addEventListener('mousemove', this.CanvasMouseMove);
		if(_nodes.resultGroup)_nodes.resultGroup.addEventListener('mouseover', this.ResultListOperation);
		if(_nodes.toolTagsManager)_nodes.toolTagsManager.addEventListener('click', this.ManageLabels)
	};
	...
	//----设置功能参数
	SetFeatures = (f, value) => {
		if (f === "crossOn" || f === "labelOn" || f === "editOn") {
			this.Features[f] = value;
		}
		else {
			for (let key in this.Features) {
				if (key !== "crossOn" && key !== "labelOn" && key !== "editOn") {
	...
	CanvasDblclick = (e) => {
		if(this.Features.dragOn){
			let _arrays = this.Arrays;
			let dot = {x:this.mouseX,y:this.mouseY};
			let bInside = false;
			let bInsideItem = null;
			_arrays.imageAnnotateShower.forEach((item, index) => {
				var arr = item.content.concat(item.content[0]);
				if(this.judgeBeInside(dot,arr)){
					bInside = true;
					bInsideItem = item;
					return false;
				}
			})
			if(bInside&&bInsideItem&&bInsideItem.id)$('.result_list_'+bInsideItem.id).get(0).click();
		}
	...
	CanvasMouseDown = (e) => {
		...
		if (e.button === 0) {
			...
			if (this.Features.polygonOn||this.Features.rectOn) {
				this.SetFeatures(this.Features.rectOn?'rectOn':'polygonOn', true);//完成标注
			}else{
				// 长按右击直接开启拖拽模式
				let prevP = this.CalculateChange(e, _nodes.canvas);
				this.prevX = prevP.x;
	...
	DrawCircle = (ctx, x, y, color) => {
		if(!this.Features.editOn)return;
		ctx.beginPath();
		ctx.fillStyle = "#000";
	...
	DrawSavedAnnotateInfoToShow = (resultIndex) => {
		...
		_arrays.imageAnnotateShower.forEach((item, index) => {
			if (item.contentType === "polygon") {
				...
				_nodes.ctx.fillStyle = "rgba("+ item.labels.labelColorRGB +"," + this.opacityPolygon + ")";
				_nodes.ctx.fill();
				_nodes.ctx.strokeStyle = "rgba("+ item.labels.labelColorRGB +"," + this.opacityPolygon + ")";
				_nodes.ctx.stroke();
			}
			...
			if (item.content.length >= 2 && item.labels.visibility) {
				...
				//this.DrawRectLabel(_nodes.ctx, item.rectMask.xMin, item.rectMask.yMin, item.labels.labelColor, item.labels.labelName, index + 1);
			}
			// 绘制矩形蒙层
		...
		if (isRender) {
			_arrays.imageAnnotateMemory.forEach( (item, index) => {
				...
				if (item.content.length >= 2 && item.labels.visibility) {
					...
					//this.DrawRectLabel(_nodes.bCtx, item.rectMask.xMin, item.rectMask.yMin, item.labels.labelColor, item.labels.labelName, index + 1);
				}
			});
		}
		this.UpdateCanvas();
	...
	RepaintResultList = () => {
		...
		if (this.Arrays.imageAnnotateShower.length > 0) {
			...
			this.Arrays.imageAnnotateShower.forEach((item, index) => {
				...
				resultListBody.className = "result_list result_list_"+item.id;
			...
			this.ResultListOperation();
		}
		if(!this.Features.editOn){
			$('.editLabelName').hide();
			$('.deleteLabel').hide();
		}
	};
	...
	CreateNewResultList = (lx, ly, contentType) => {
		...
		let defItem = {labelName:'未命名'};
		if (labels.length > 0)defItem = labels[0];

		resultListBody.className = "result_list active result_list_"+id;
		...
		resultListBody.innerHTML = '<span class="result_no">'+ resultLength +'</span>' +
			...
			'<span class="result_color" style="background: '+ defItem.labelColor +';"></span>' +
			'<input class="result_Name" value="'+defItem.labelName+'" disabled>' +
			'<i class="editLabelName icon-pencil"></i>' +
			'<i class="deleteLabel icon-trash"></i>' +
			'<i class="isShowLabel '+ eyeIconClass +'"></i>';
		...
		//默认选中第一个标签
		if(labels.length >0){
			_arrays.imageAnnotateShower[resultLength-1].labels.labelName = defItem.labelName;
			_arrays.imageAnnotateShower[resultLength-1].labels.labelColor = defItem.labelColor;
			_arrays.imageAnnotateShower[resultLength-1].labels.labelColorRGB = defItem.labelColorR + "," + defItem.labelColorG + "," + defItem.labelColorB;
			_arrays.imageAnnotateShower[resultLength-1].labels.categoryId = defItem.id;
		}
	...
	ResultListOperation = () => {
		...
		for (let i=0; i<resultList.length; i++) {
			...
			resultList[i].onclick = function(event) {
				...
				//非编辑状态下根据绘制类型跳转到相应绘制功能
				if(!_self.Features.dragOn){
					let featureItem = 'toolRect';
					if(_self.Arrays.imageAnnotateShower[this.index].contentType=='polygon')featureItem = 'toolPolygon';
					$('.'+featureItem).click();
				}
		...
		if (labels.length > 0) {
			...
			labels.forEach((item, index) => {
				...
				labelLi.onclick = function() {
					...
					_self.Arrays.imageAnnotateShower[resultIndex].labels.categoryId = item.id;
	...
	//----判断点击位置是否处于多边形内
	judgeBeInside = (dot,coordinates,noneZeroMode) => {
		// 默认启动none zero mode
